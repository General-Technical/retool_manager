...
is_environment_enabled() {
    ...
    if [ "$env" = "prod" ] && [ "$USE_PROD_ENV" != "true" ]; then
        return 1
    elif [ "$env" = "staging" ] && [ "$USE_STAGING_ENV" != "true" ]; then
        return 1
    elif [ "$env" = "dev" ] && [ "$USE_DEV_ENV" != "true" ]; then
        return 1
    elif [ "$env" = "all" ]; then
        return 0
    fi
...
backup_environment() {
    ...
    if docker compose ps -q postgres &>/dev/null; then
        ...
        docker compose exec -T postgres pg_dump -U retool retool > $backup_path/database_backup.sql 2>/dev/null || \
...
update_environment() {
    ...
    if [ -f "$env_dir/Dockerfile" ]; then
        sed -i "s/ARG VERSION=.*/ARG VERSION=$version/" $env_dir/Dockerfile
    else
        log_message "ERROR" "Dockerfile not found in $env_dir"
        exit 1
    fi
...
show_status() {
    ...
    if docker compose ps -q postgres &>/dev/null; then
        ...
        if docker compose exec -T postgres pg_isready -U retool >/dev/null 2>&1; then
            ...
            docker compose exec -T postgres psql -U retool -c "SELECT pg_size_pretty(pg_database_size('retool'));" || \
...
promote_environment() {
    ...
    if [ -f "$target_dir/Dockerfile" ]; then
        sed -i "s/ARG VERSION=.*/ARG VERSION=$source_version/" $target_dir/Dockerfile
    else
        log_message "ERROR" "Target Dockerfile not found"
        exit 1
    fi
    ...
    if docker compose ps -q api &>/dev/null; then
        ...
        # Example: curl -H "Authorization: Bearer $API_KEY" http://localhost:3000/api/export-apps > $backup_path/apps.json
    else
...
create_environment() {
    ...
    if [ -n "$api_port" ] && [ -n "$http_port" ] && [ -n "$https_port" ]; then
        ...
        sed -i "s/\"3000:3000\"/\"$api_port:3000\"/" docker-compose.yml
    ...
    if [ "$env_name" = "development" ]; then
        ...
        echo "COOKIE_INSECURE=false" >> docker.env
    elif [ "$env_name" = "production" ]; then
        echo "NODE_ENV=production" >> docker.env
        echo "COOKIE_INSECURE=false" >> docker.env
    fi
...
check_health() {
    ...
    if [ $running_containers -lt $expected_services ]; then
        ...
        docker compose ps --services --filter "status=exited" --filter "status=stopped" --filter "status=created"
        
        # Check for common issues
        log_message "INFO" "Checking logs for common issues..." $HEALTH_LOG
        docker compose logs --tail=50 | grep -E "ERROR|FATAL|Exception|failed"
    else
    ...
    # Check API health
    local api_container=$(docker compose ps -q api 2>/dev/null)
...
rollback_environment() {
    ...
    if [ -f "$backup_path/database_backup.sql" ]; then
        ...
        cat $backup_path/database_backup.sql | docker compose exec -T postgres psql -U retool retool
...
optimize_database() {
    ...
    if docker compose exec -T postgres psql -U retool -c "VACUUM ANALYZE;" retool; then
...
case $COMMAND in
    status)
        if [ "$ENVIRONMENT" = "all" ]; then
            # Only show enabled environments
            if [ "$USE_PROD_ENV" = "true" ]; then
                show_status "production" $PROD_DIR
            fi
            if [ "$USE_STAGING_ENV" = "true" ]; then
                show_status "staging" $STAGING_DIR
            fi
            if [ "$USE_DEV_ENV" = "true" ]; then
                show_status "development" $DEV_DIR
            fi
        elif [ "$ENVIRONMENT" = "prod" ]; then
            if [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Production environment is disabled in configuration"
                exit 1
            fi
            show_status "production" $PROD_DIR
        elif [ "$ENVIRONMENT" = "staging" ]; then
            if [ "$USE_STAGING_ENV" != "true" ]; then
                log_message "ERROR" "Staging environment is disabled in configuration"
                exit 1
            fi
            show_status "staging" $STAGING_DIR
        elif [ "$ENVIRONMENT" = "dev" ]; then
            if [ "$USE_DEV_ENV" != "true" ]; then
                log_message "ERROR" "Development environment is disabled in configuration"
                exit 1
            fi
            show_status "development" $DEV_DIR
        else
            log_message "ERROR" "Unknown environment '$ENVIRONMENT'. Use 'prod', 'staging', 'dev', or 'all'"
            exit 1
        fi
        ;;
    ...
    update)
        if [ "$ENVIRONMENT" = "all" ]; then
            if [ "$USE_PROD_ENV" = "true" ]; then
                update_environment "production" $PROD_DIR $PROD_VERSION_POLICY $PROD_SPECIFIC_VERSION
            fi
            if [ "$USE_STAGING_ENV" = "true" ]; then
                update_environment "staging" $STAGING_DIR $STAGING_VERSION_POLICY $STAGING_SPECIFIC_VERSION
            fi
            if [ "$USE_DEV_ENV" = "true" ]; then
                update_environment "development" $DEV_DIR $DEV_VERSION_POLICY $DEV_SPECIFIC_VERSION
            fi
        elif [ "$ENVIRONMENT" = "prod" ]; then
            if [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Production environment is disabled in configuration"
    ...
    backup)
        if [ "$ENVIRONMENT" = "all" ]; then
            if [ "$USE_PROD_ENV" = "true" ]; then
                update_environment "production" $PROD_DIR $PROD_VERSION_POLICY $PROD_SPECIFIC_VERSION backup_only
            fi
            if [ "$USE_STAGING_ENV" = "true" ]; then
                update_environment "staging" $STAGING_DIR $STAGING_VERSION_POLICY $STAGING_SPECIFIC_VERSION backup_only
            fi
            if [ "$USE_DEV_ENV" = "true" ]; then
                update_environment "development" $DEV_DIR $DEV_VERSION_POLICY $DEV_SPECIFIC_VERSION backup_only
            fi
        elif [ "$ENVIRONMENT" = "prod" ]; then
            if [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Production environment is disabled in configuration"
    ...
    promote)
        if [ "$ENVIRONMENT" = "dev-to-staging" ]; then
            if [ "$USE_DEV_ENV" != "true" ] || [ "$USE_STAGING_ENV" != "true" ]; then
                log_message "ERROR" "Cannot promote: either development or staging environment is disabled"
                exit 1
            fi
            promote_environment "development" "staging" $DEV_DIR $STAGING_DIR
        elif [ "$ENVIRONMENT" = "staging-to-prod" ]; then
            if [ "$USE_STAGING_ENV" != "true" ] || [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Cannot promote: either staging or production environment is disabled"
                exit 1
            fi
            promote_environment "staging" "production" $STAGING_DIR $PROD_DIR
        elif [ "$ENVIRONMENT" = "dev-to-prod" ]; then
            if [ "$USE_DEV_ENV" != "true" ] || [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Cannot promote: either development or production environment is disabled"
                exit 1
            fi
    ...
    start)
        if [ "$ENVIRONMENT" = "all" ]; then
            if [ "$USE_PROD_ENV" = "true" ]; then
                start_environment "production" $PROD_DIR
            fi
            if [ "$USE_STAGING_ENV" = "true" ]; then
                start_environment "staging" $STAGING_DIR
            fi
            if [ "$USE_DEV_ENV" = "true" ]; then
                start_environment "development" $DEV_DIR
            fi
        elif [ "$ENVIRONMENT" = "prod" ]; then
            if [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Production environment is disabled in configuration"
    ...
    stop)
        if [ "$ENVIRONMENT" = "all" ]; then
            if [ "$USE_PROD_ENV" = "true" ]; then
                stop_environment "production" $PROD_DIR
            fi
            if [ "$USE_STAGING_ENV" = "true" ]; then
                stop_environment "staging" $STAGING_DIR
            fi
            if [ "$USE_DEV_ENV" = "true" ]; then
                stop_environment "development" $DEV_DIR
            fi
        elif [ "$ENVIRONMENT" = "prod" ]; then
            if [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Production environment is disabled in configuration"
    ...
    health)
        if [ "$ENVIRONMENT" = "all" ]; then
            if [ "$USE_PROD_ENV" = "true" ]; then
                check_health "production" $PROD_DIR
            fi
            if [ "$USE_STAGING_ENV" = "true" ]; then
                check_health "staging" $STAGING_DIR
            fi
            if [ "$USE_DEV_ENV" = "true" ]; then
                check_health "development" $DEV_DIR
            fi
        elif [ "$ENVIRONMENT" = "prod" ]; then
            if [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Production environment is disabled in configuration"
    ...
    rollback)
        ...
        if [ "$ENVIRONMENT" = "prod" ]; then
            ...
            rollback_environment "production" $PROD_DIR $BACKUP_ID
        elif [ "$ENVIRONMENT" = "staging" ]; then
            if [ "$USE_STAGING_ENV" != "true" ]; then
                log_message "ERROR" "Staging environment is disabled in configuration"
                exit 1
            fi
            rollback_environment "staging" $STAGING_DIR $BACKUP_ID
        elif [ "$ENVIRONMENT" = "dev" ]; then
            if [ "$USE_DEV_ENV" != "true" ]; then
                log_message "ERROR" "Development environment is disabled in configuration"
                exit 1
            fi
            rollback_environment "development" $DEV_DIR $BACKUP_ID
        else
            log_message "ERROR" "Unknown environment '$ENVIRONMENT'. Use 'prod', 'staging', or 'dev'"
            exit 1
        fi
    ...
    logs)
        ...
        if [ "$ENVIRONMENT" = "prod" ]; then
            ...
            view_logs "production" $PROD_DIR $SERVICE $LINES
        elif [ "$ENVIRONMENT" = "staging" ]; then
            if [ "$USE_STAGING_ENV" != "true" ]; then
                log_message "ERROR" "Staging environment is disabled in configuration"
                exit 1
            fi
            view_logs "staging" $STAGING_DIR $SERVICE $LINES
        elif [ "$ENVIRONMENT" = "dev" ]; then
            if [ "$USE_DEV_ENV" != "true" ]; then
                log_message "ERROR" "Development environment is disabled in configuration"
                exit 1
            fi
            view_logs "development" $DEV_DIR $SERVICE $LINES
        else
            log_message "ERROR" "Unknown environment '$ENVIRONMENT'. Use 'prod', 'staging', or 'dev'"
            exit 1
        fi
    ...
    list-backups)
        if [ "$ENVIRONMENT" = "all" ]; then
            ...
            if [ "$USE_STAGING_ENV" = "true" ]; then
                list_backups "staging"
            fi
            if [ "$USE_DEV_ENV" = "true" ]; then
                list_backups "development"
            fi
        elif [ "$ENVIRONMENT" = "prod" ]; then
            if [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Production environment is disabled in configuration"
    ...
    optimize-db)
        if [ "$ENVIRONMENT" = "all" ]; then
            if [ "$USE_PROD_ENV" = "true" ]; then
                optimize_database "production" $PROD_DIR
            fi
            if [ "$USE_STAGING_ENV" = "true" ]; then
                optimize_database "staging" $STAGING_DIR
            fi
            if [ "$USE_DEV_ENV" = "true" ]; then
                optimize_database "development" $DEV_DIR
            fi
        elif [ "$ENVIRONMENT" = "prod" ]; then
            if [ "$USE_PROD_ENV" != "true" ]; then
                log_message "ERROR" "Production environment is disabled in configuration"
    ...
    env-config)
        manage_env_config "$ENVIRONMENT" "$OPTION"
        ;;
